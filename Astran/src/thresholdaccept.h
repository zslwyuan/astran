...
template <class toOptimize> int FindInitialThreshold(toOptimize &obj)
{
    ...
    do
    {
        ...
        !((avg_delta / ini_cost < 0.000000001 && avg_delta / ini_cost > -0.000000001) || ((temp_max - temp_min) <= 1)));
...
toOptimize ThresholdAccept(toOptimize &initial_solution, double quality_factor, bool greedy = false,
                           bool print_progress = true, bool high_threshold = false)
{
    ...
    if (print_progress)
        std::cout << " ( initial cost = " << cost << " num reps = " << num_repetitions
                  << " problem size = " << initial_solution.getProblemSize() << " ) ";
    ...
    for
        ever
        {
            ...
            if (print_progress)
                printf(" Iteration %i; cost = %.3f (%.3f); p_cost = %.3f; accept = %.3f; avg_delta = %.3f; dp = %.3f; "
                       "thres = %.3f; cost_ratio = %.3f; [%i]\n",
                       iter, cost, static_cast<double>(bestSolution.getCost()), previous_cost, accept_ratio, avg_delta,
                       desvio_padrao, threshold, cost_ratio, cont_same_cost);

            if (print_progress)
                std::cerr << ".";
            if (time_of_iteration > 80)
                break;
